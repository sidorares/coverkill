...
import type { ByteRange, V8FunctionCoverage } from './types.js';

/**
 * The minimum a V8 `ScriptCoverage` needs for classification: only the counts
 * matter, so both report-v2 scripts and raw Playwright/CDP payloads fit.
 */
export type V8ScriptCoverageLike = {
  url?: string;
  scriptId?: string;
  source?: string;
 ...
 *
 * This is prune-time policy: it runs on the raw counts a report v2 carries, so
 * a different stub/delete policy can be applied to an old report.
 */
export function extractJsCoverage(entry: V8ScriptCoverageLike): {
  covered: ByteRange[];
  stub: ByteRange[];
} {
  let maxEnd = 0;
 ...
 */
function classifyStubs(entry: V8ScriptCoverageLike, uncovered: Segment[]): ByteRange[] {
  const fns: FnSpan[] = [];
  for (const fn of entry.functions) {
